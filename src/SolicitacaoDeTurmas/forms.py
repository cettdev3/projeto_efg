...
class SolicitacaoDeTurmas(forms.ModelForm):
    udepi = forms.ChoiceField(
        choices=(("SIM", "Sim"), ("NAO", "Não")), widget=forms.RadioSelect
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Row(
                Column(
                    ...
                    onChange="populateDependentLists('{}')".format("id_eixo", "/eixo"),
                ),
                css_class="form-row",
            ),
            Row(
        ...
        self.fields["unidade_ensino"].queryset = Metas_escolas.objects.filter(tipo=2)

        self.fields["eixo"].queryset = (
            Eixos.objects.order_by().values_list("nome", flat=True).distinct()
        )

        self.fields["curso"].queryset = (
            Cursos.objects.order_by().values_list("curso", flat=True).distinct()
        )
