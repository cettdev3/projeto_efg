...
def view_eixos(request):
    ...
    cursos = []
    return render(
        request, "form-element-input.html", {"eixos": eixos, "cursos": cursos}
    )
...
def load_cht(request):
    ...
    return render(
        request, "ajax/ajax_load_cht.html", {"ch_total_geral": ch_total_geral}
    )
...
def load_ch(request):
    ...
    return render(
        request, "ajax/ajax_load_carga_hr_curso.html", {"carga_hr": carga_horaria}
    )
...
def load_modalidade(request):
    ...
    modalidade_id = request.GET.get("modalidade_id")
    cursos = (
        Cadastrar_curso.objects.filter(
            escola=escola_id,
            tipo=tipo_id,
            eixos=eixos_id,
            modalidade=modalidade_id,
...
def load_funcoes_filter(request):
    ...
    return render(
        request,
        "ajax/ajax_load_filter.html",
        {"results": results, "infos": info, "vagas": vagas},
    )
...
def load_funcoes_vagas(request):
    ...
    vagas = Metas_efg.objects.filter(**filters).aggregate(
...
def load_funcoes_gerencia_cursos(request):
    ...
    status = request.GET["status"]
    cursos = (
        Cadastrar_curso.objects.select_related("escola")
        .select_related("tipo")
        .select_related("eixos")
        .get(id=id)
    )
    if status == "ATIVO":
...
def load_funcoes_permissoes(request):
    ...
    return render(
        request, "ajax/ajax_load_permissoes.html", {"perm": perm, "escolas": escolas}
    )
...
def load_funcoes_rp(request):
    ...
    if tipo:
        filters["tipo_curso"] = tipo
    total_horas = Metas_efg.objects.filter(**filters).aggregate(
...
def load_funcoes_chd(request):
    ...
    if tipo:
        filters["tipo"] = tipo
    total_horas = DivisaoDeMetasPorEscola.objects.filter(**filters).aggregate(
    ...
    return render(
        request, "ajax/ajax_load_carga_hr_disponivel.html", {"chd": total_horas}
    )
...
def load_funcoes_total_horas(request):
    ...
    # total_horas = int(horas) * int(vagas)
    return render(
        request, "ajax/ajax_load_total_horas.html", {"total_horas": total_horas}
    )
...
def cadastrar_metas(request):
    ...
    if perm_escolas is not None:
        ...
        btn_enviar_planejamento = Metas_efg.objects.filter(
            escola_id=int(perm_escolas)
        ).values()

        btn_enviar_planejamento_reprovados = (
            Metas_efg.objects.filter(escola_id=int(perm_escolas), situacao=1)
            .all()
            .count()
        )

        btn_enviar_planejamento_aanalise = (
            Metas_efg.objects.filter(escola_id=int(perm_escolas), situacao=0)
            .all()
            .count()
        )
    ...
    if perm_escolas is not None:
        return render(
            ...
            "cadastro_metas.html",
            {
                ...
                "saldoReplanejado": saldoReplanejado,
            },
        )
    else:
        return render(
...
def cad_metas(request):
    ...
    if True:
        ...
        if (
            request.POST["p_abertura_edital"] != ""
            and request.POST["p_fechamento_edital"] != ""
        ):
        ...
        meta_is_exist = Metas_efg.objects.filter(
            escola_id=escola,
        ...
        if meta_is_exist:
            ...
            if "btn-check-2-outlined" in request.POST:
                ...
                messages.success(
                    request,
                    "A meta cadastrada com replanejamento foi realizada com sucesso!",
                )
                return redirect("/cadastrar-metas")
            else:
...
def apagar_meta(request):
    ...
    id_filtro = atualiza_saldo[0]["id"]
    novo_saldo = int(atualiza_saldo[0]["carga_horaria"]) + int(ch_total)
...
def editar_meta(request, codigo):
    ...
    atualiza_saldo = DivisaoDeMetasPorEscola.objects.filter(
        escola=infoFiltro[0]["escola_id"],
        tipo=infoFiltro[0]["tipo_curso_id"],
        ano=infoFiltro[0]["ano"],
    ).values()
...
def editarmetas(request):
    ...
    # CARGA HORÁRIA DISPONÍVEL DA META
    cht_disponivel_divisao = int(atualiza_saldo[0]["carga_horaria"]) + int(
        cht_original_meta
    )
...
def cadastrar_meta_sintetica(request):
    ...
    return render(
        request,
        "cadastro_metas_sinteticas.html",
        {
            "meta_sinteticas": meta_sintetica,
            "escolas": escolas,
...
def cad_metas_sintetica(request):
    ...
    cadmetas = Metas_sinteticas.objects.create(
        diretoria=diretoria,
        escola_id=escola,
        ano=ano,
...
def orcamento_plano_trabalho(request):
    ...
    return render(
        request,
        "orcamento_plano_trabalho.html",
        {
            "rubricas": rubrica,
            "centro_custo": centro_custo,
            "unidades": unidades,
            "planejamentos": planejamento,
            "itens_apoiados": item_apoiado,
            "permissoes": get_permission(request),
        },
    )
...
def rubrica(request):
    ...
    return render(
        request,
        "orcamento_plano_trabalho.html",
        {"rubricas": rubrica, "centro_custo": centro_custo},
    )
...
def load_funcoes_rubrica(request):
    ...
    return render(
        request, "ajax/ajax_load_item_apoiado.html", {"itens_apoiados": item_apoiado}
    )
...
def cadastro_curso_escola(request):
    ...
    return render(
        request,
        "cadastro_curso_escola.html",
        {"escolas": escolas, "cursos": curso, "permissoes": get_permission(request)},
    )
...
def cadastrar_curso(request):
    ...
    return render(
        ...
        "cadastro_curso.html",
        {
            ...
            "tipos": tipos,
            "eixos": eixos,
            "escolas_cursos": escolas_cursos,
            "tipo_curso": tipo_cursos,
            "eixos_curso": eixos_curso,
            "eixos_inicial": eixos_inicial,
            "modalidades": modalidade,
            "escolas": escolas,
            "permissoes": get_permission(request),
        },
...
def cad_novo_curso(request):
    ...
    cadCurso = Cadastrar_curso.objects.create(
        escola_id=escola,
        tipo_id=tipo,
        eixos_id=eixos,
        curso=curso,
...
def cadastrar_udepi(request):
    ...
    return render(
        request, "cadastrar_udepi.html", {"escolas": escolas, "udepis": udepis}
    )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "aprovar_selecionados" in self.request.POST:
            ...
            for id in range(0, len(id_list)):
                Metas_efg.objects.filter(pk=id_list[id]).update(
                    situacao=3, jus_reprovacao=""
                )
        ...
        if "aprovar" in self.request.POST:
            ...
            for id in range(0, len(id_list)):
                if action_list[id] != "1":
                    Metas_efg.objects.filter(pk=id_list[id]).exclude(
                        situacao=action_list[id]
                    ).update(situacao=action_list[id], jus_reprovacao="")
                else:
            ...
            if len(tasklist) > 0 and ApprovalLen == 1 and ApprovalType == 3:
                for task in tasklist:
                    complete = CamundaTask.Complete(
                        url=config.CAMUNDA_URL, id_=task.id_
                    )
                    complete.add_variable(
                        name="aprovacaodoscursos", value=0, type_="Integer"
                    )
                ...
                messages.success(self.request, "Planejamento de turmas foi aprovado")
            elif len(tasklist) == 0 and ApprovalLen == 1:
                messages.error(
                    self.request,
                    "Não foi encontrado tarefas para de aprovar o planejamento de turma",
                )
            elif ApprovalLen > 1:
                messages.warning(
                    self.request,
                    "Para ter aprovação do planejamento de turmas, todas as turmas deveram ser aprovadas",
                )
...
class AprovarCursosUpdateView(
    LoginRequiredMixin, SuccessMessageMixin, UpdateView, AuthenticationContextMixin
):
    login_url = "/"
...
class ReprovaCursosUpdateView(
    LoginRequiredMixin, SuccessMessageMixin, FormView, AuthenticationContextMixin
):
    login_url = "/"
    ...
    def form_valid(self, form):
        ...
        if "pks" in params:  # type: ignore
            selected_objects = Metas_efg.objects.filter(id__in=params["pks"]).exclude(
                situacao="1"
            )  # type: ignore
            selected_objects.update(
                situacao=params["action"],
                jus_reprovacao=form.cleaned_data["jus_reprovacao"],
            )  # type: ignore
            ...
            if len(tasklist) > 0:
                for task in tasklist:
                    ...
                    complete.add_variable(
                        name="aprovacaodoscursos", value=1, type_="Integer"
                    )

                    escolas = Metas_efg.objects.filter(
                        id__in=params["pks"]
                    ).values_list("escola", flat=True)
                    mails = json.dumps(
                        list(
                            Metas_escolas.objects.filter(id__in=escolas).values("email")
                        )
                    )

                    print(mails)
                    complete.add_variable(
                        name="emailsEscolasReprovadas", value=mails, type_="Json"
                    )
...
class DashboardAprovarCursosView(
    LoginRequiredMixin, AuthenticationContextMixin, FilterView
):
    login_url = "/"
...
def get_cursos(request):
    ...
    cursos = (
        Cadastrar_curso.objects.filter(**filtro).values("id", "curso").order_by("curso")
    )
...
def verifica_turmas_edital(request):
    ...
    return render(
        request,
        "verificacao_turmas_edital.html",
        {"metas": metas, "permissoes": get_permission(request)},
    )
...
def ajax_load_turmas_edital(request):
    ...
    return render(
        request,
        "ajax/ajax_load_turmas_edital.html",
        {"edital": edital, "lancamentos": lancamentos},
    )
...
def ajax_load_turmas_edital_filtro(request):
    ...
    return render(
        request,
        "ajax/ajax_load_turmas_edital_filtro.html",
        {"lancamentos": lancamentos},
    )
...
def gerenciar_usuarios(request):
    ...
    return render(
        request,
        "user_perm.html",
        {"users": users, "permissoes": get_permission(request), "escolas": escolas},
    )
...
def enviar_planejamento(request):
    ...
    if int(status_metas["situacao"]) == 2 or int(status_metas["situacao"]) == 3:
        messages.error(
            request,
            "Não foi possível enviar o planejamento, o mesmo encontra-se aprovado ou está em status de análise!",
        )
        return redirect("/cadastrar-metas")
    else:
        ...
        escola_bittencourt_1 = Metas_efg.objects.filter(
            escola_id=54, situacao=2
        ).count()
        escola_bittencourt_2 = Metas_efg.objects.filter(
            escola_id=54, situacao=3
        ).count()

        status_luiz_rassi = (
            True if escola_luiz_rassi_1 + escola_luiz_rassi_2 > 0 else False
        )
        status_sara = True if escola_sara_1 + escola_sara_2 > 0 else False
        status_bittencourt = (
            True if escola_bittencourt_1 + escola_bittencourt_2 > 0 else False
        )

        # print( escola_luiz_rassi,escola_sara,escola_bittencourt)
        if (
            status_luiz_rassi is True
            and status_sara is True
            and status_bittencourt is True
        ):
            ...
            if sendPlan is True:
                ...
                messages.error(request, "Não foi possível enviar o planejamento!")
                return redirect("/cadastrar-metas")
        else:
...
def replanejar_curso(request):
    ...
    saldo_disponivel = Saldo_replanejamento.objects.filter(
        modalidade_id=modalidade, tipo_id=tipo, ano=ano, semestre=semestre
    ).values()
    if saldo_disponivel:
...
def buscar_saldo_replanejado(request):
    ...
    return render(
        request,
        "ajax/ajax_select_saldo_replanejado.html",
        {"saldo_especifico": saldo_especifico},
    )
