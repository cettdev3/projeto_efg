...
def camundaPutVariable(taskId, variable, variableValue, type):
    ...
    putVariable = req.put(
        f"{host}/task/{taskId}/variables/{variable}",
        json=bodyJson,
        auth=autentication,
        headers=headers,
    )
    ...
    for dados in json_object:
        ...
        try:
            if dados["taskDefinitionKey"] == taskDefinitionKey:
                ...
                putUserLogin = camundaPutVariable(
                    idTask, "UserDjangoAuth", userLogged, "String"
                )
                putUserSelecao = camundaPutVariable(
                    idTask, "UserSelecaoAuth", idUserSelecao, "String"
                )
                putUserSiga = camundaPutVariable(
                    idTask, "UserSigaAuth", idUserSiga, "String"
                )

                if (
                    putvariable is True
                    and putUserSelecao is True
                    and putUserLogin is True
                    and putUserSiga is True
                ):
...
def ajax_load_edital_v2(request):
    ...
    if tipo_edital == "normal":
        ...
        lancamentos = (
            Editais_Retificados.objects.filter(id=edital_id)
    ...
    return render(
        request,
        "ajax_load_edital_v2.html",
        {"edital": edital, "tipo_edital": tipo_edital},
    )
...
def aprovar_edital_gerado(request):
    ...
    edital = (
        Edital.objects.filter(num_edital=num_edital).values()
        if tipo_edital == "normal"
        else Editais_Retificados.objects.filter(num_edital=num_edital).values()
    )
    print("----------------------------")
    print(edital[0])
    ...
    if aprovacao == 3:
        ...
        atualiza_status = (
            Edital.objects.get(id=idEdital)
            if tipo_edital == "normal"
            else Editais_Retificados.objects.get(id=idEdital)
        )
        ...
        todosEditais = (
            Edital.objects.filter(~Q(status=3) | Q(status=4)).values()
            if tipo_edital == "normal"
            else Editais_Retificados.objects.filter(~Q(status=3) | Q(status=4)).values()
        )
        ...
        if len(todosEditais) > 0:
            ...
            completeTask = getInstance(
                processName,
                "ConferiraprovarOEditalChecklistSGEVariavelEditalTemAlteracoesTask",
                request,
            )
            messages.success(
                request, "Edital foi aprovado com sucesso! Processo em andamento..."
            )
            return redirect("/aprovar-edital")
